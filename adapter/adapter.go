...
import (
	...
	"github.com/DODOEX/oracle-adapter/connector"
...
func getExchangeResponses(base, quote string) ([]*connector.Response, []error) {
	exchanges := getExchangesWithPairSupport(base, quote)

	var wg sync.WaitGroup
	wg.Add(len(exchanges))

	var responses []*connector.Response
	var errs []error

	mutex := sync.Mutex{}
	for _, exc := range exchanges {
		go func(exc connector.Interface) {
			defer wg.Done()
...
func getQuoteUSDPrice(quote string) (float64, []error) {
	...
	if len(responses) == 0 {
		errs = append(errs, &connector.Error{
			Exchange: "N/A",
...
func aggregateResponses(responses []*connector.Response) (float64, float64) {
	var price float64
	var volume float64
	for _, response := range responses {
		if response != nil {
...
func getExchangesWithPairSupport(base, quote string) []connector.Interface {
	exchanges := connector.GetSupportedExchanges()

	var wg sync.WaitGroup
	wg.Add(len(exchanges))

	mutex := sync.Mutex{}
	var supported []connector.Interface
	for _, exc := range exchanges {
		go func(exc connector.Interface) {
			defer wg.Done()
